...
use crate::jira::comments::CommentsService;

pub trait DetailService: Interface {
    fn show_details(&self, ticket: String, fields: String);
}

pub struct DefaultDetailService {
    jira_api: Arc<dyn JiraApi>,
    comments_service: Arc<dyn CommentsService>,
}

impl DefaultDetailService {
    pub fn new(jira_api: Arc<dyn JiraApi>, comments_service: Arc<dyn CommentsService>) -> Self {
        Self {
            jira_api,
            comments_service,
        }
    ...
    fn show_detail_field(&self, value: &json::JsonValue, field: String) {
        let hide_keys = [
            "key".to_string(),
            "summary".to_string(),
        ...
        if field == "comment" {
            println!();
            self.comments_service.display_comment_list(value);
            return;
        }
        if field == "parent" {
            println!(
...
impl DetailService for DefaultDetailService {
    fn show_details(&self, ticket: String, fields: String) {
        ...
        let details_response = self.jira_api.get_v2(&format!("issue/{ticket}"));

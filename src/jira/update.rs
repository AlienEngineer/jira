...
pub trait UpdateService: Interface {
    fn update_jira_ticket(&self, ticket: String, key: String, entry: String);
}

pub struct DefaultUpdateService {
...
impl UpdateService for DefaultUpdateService {
    fn update_jira_ticket(&self, ticket: String, key: String, entry: String) {
        ...
        if fields["allowedValues"].is_array() {
            if fields["schema"]["type"] == "array" {
                let update_json_value =
                    get_object_lists_from_value(&fields["allowedValues"], value);
